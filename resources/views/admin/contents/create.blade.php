...
@section('page_title', __('Add Content'))
...
@php
    $initialItems = collect(old('items', []))->map(function($item) {
        return [
            ...
            'show_button' => !empty($item['button_text_ar']),
            'imagePreview' => $item['image'] ?? null,
        ];
    });
@endphp

<div class="max-w-4xl mx-auto pb-20" dir="rtl" x-data="contentManager({{ json_encode($initialItems) }})">
    {{-- Header --}}
    <div class="flex items-center gap-4 mb-10">
        ...
        <h2 class="text-2xl font-black text-[#1A1A31] dark:text-white">{{ __('Add Content') }}</h2>
    </div>
    ...
    <form action="{{ route('admin.contents.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8" @submit.prevent="submitForm">
        @csrf
        ...
        <div class="bg-white dark:bg-[#1A1A31] rounded-[2.5rem] p-10 shadow-sm border border-slate-50 dark:border-white/5 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-3">
                    ...
                    <input type="text" name="title_ar" value="{{ old('title_ar') }}" required placeholder="{{ __('Enter title') }}" class="w-full px-8 py-5 rounded-2xl bg-[#F8F9FE] dark:bg-white/5 border border-slate-100 dark:border-white/10 text-sm font-bold text-[#1A1A31] dark:text-white outline-none focus:ring-4 focus:ring-[#1A1A31]/5 transition-all @error('title_ar') border-red-500 @enderror">
                ...
                <div class="space-y-3" dir="ltr">
                    ...
                    <input type="text" name="title_en" value="{{ old('title_en') }}" placeholder="Enter title" class="w-full px-8 py-5 rounded-2xl bg-[#F8F9FE] dark:bg-white/5 border border-slate-100 dark:border-white/10 text-sm font-bold text-[#1A1A31] dark:text-white outline-none focus:ring-4 focus:ring-[#1A1A31]/5 transition-all font-sans @error('title_en') border-red-500 @enderror">
...
document.addEventListener('alpine:init', () => {
    Alpine.data('contentManager', (initialItems) => ({
        is_visible: 1,
        items: initialItems,
        currentItem: {
            title_ar: '',
        ...
        submitForm(event) {
            ...
            // Serialize items as JSON into the hidden field
