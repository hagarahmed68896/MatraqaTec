...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-[#1A1A31] rounded-[2rem] border border-slate-100 dark:border-white/5 p-6 shadow-sm">
        <form action="{{ route('admin.payments.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Search customer name...') }}" class="px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 focus:outline-none focus:ring-2 focus:ring-primary/20 dark:text-white">
            ...
            <select name="status" class="px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 focus:outline-none focus:ring-2 focus:ring-primary/20 dark:text-white">
                ...
                <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>{{ __('Under Review') }}</option>
                <option value="failed" {{ request('status') == 'failed' ? 'selected' : '' }}>{{ __('Failed') }}</option>
            </select>

            <select name="payment_method" class="px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 focus:outline-none focus:ring-2 focus:ring-primary/20 dark:text-white">
                ...
                <option value="transfer" {{ request('payment_method') == 'transfer' ? 'selected' : '' }}>{{ __('Transfer') }}</option>
            </select>
            ...
            <table class="w-full text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                ...
                <tbody class="text-xs font-bold text-slate-600 dark:text-white/70">
                    ...
                    <tr class="border-b border-slate-50 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 transition-all">
                        ...
                        <td class="py-4 px-6">
                            @if($item->order_id)
                            <a href="{{ route('admin.orders.show', $item->order_id) }}" class="text-primary hover:underline">#{{ $item->order_id }}</a>
                            @else
                            -
                            @endif
                        </td>
                        ...
                        <td class="py-4 px-6">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.payments.show', $item->id) }}" class="p-2 rounded-lg hover:bg-blue-50 text-blue-600">
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="py-12 text-center text-slate-400">{{ __('No payments found') }}</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
