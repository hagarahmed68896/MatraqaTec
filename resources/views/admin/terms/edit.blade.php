...
@section('title', __('Edit Term Data') . ' - ' . __('MatraqaTec'))
...
<div class="space-y-8 animate-in fade-in slide-in-from-bottom duration-700">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.terms.index') }}" class="w-12 h-12 bg-white dark:bg-white/5 rounded-2xl flex items-center justify-center text-slate-400 hover:text-primary hover:bg-primary/5 transition-all shadow-sm">
            ...
            <h1 class="text-2xl font-black text-slate-800 dark:text-white">{{ __('Edit Term Data') }}</h1>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-white dark:bg-[#1A1A31] rounded-[2.5rem] p-10 shadow-sm border border-slate-100 dark:border-white/5">
        <form action="{{ route('admin.terms.update', $item->id) }}" method="POST" class="space-y-10">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="title_ar" value="{{ old('title_ar', $item->title_ar) }}" placeholder="{{ __('Enter Title') }}" required class="w-full bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 dark:text-white placeholder-slate-300 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="title_en" value="{{ old('title_en', $item->title_en) }}" placeholder="{{ __('Enter Title') }}" required class="w-full bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 dark:text-white placeholder-slate-300 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                    ...
                    <!-- Status Picker -->
                    <div class="space-y-3" x-data="{ open: false, selected: '{{ old('status', $item->status) }}' }">
                        <label class="block text-sm font-black text-slate-700 dark:text-slate-300 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                            {{ __('Status') }}
                        </label>
                        <div class="relative">
