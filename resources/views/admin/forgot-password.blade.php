...
<head>
    ...
    <title>{{ __('Forgot Password?') }} - {{ __('MatraqaTec') }}</title>
            ...
            <div class="w-full lg:w-1/2 p-10 lg:p-12 flex flex-col justify-between">
                ...
                <div class="max-w-md mx-auto w-full flex-grow flex flex-col justify-center py-6">
                    <div class="mb-8 text-center">
                        <h1 class="text-3xl font-black text-slate-900 dark:text-white mb-2 tracking-tight leading-tight">{{ __('Forgot Password?') }}</h1>
                        <p class="text-slate-500 dark:text-slate-400 text-sm font-medium leading-relaxed">
                            {{ __('Enter your registered phone number and we will send you a verification code.') }}
                        </p>
                    </div>
                    ...
                    <form action="{{ route('admin.post-forgot-password') }}" method="POST" class="space-y-6">
                        @csrf
                        <div class="space-y-3">
