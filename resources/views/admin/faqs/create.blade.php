...
@section('title', __('Add New FAQ') . ' - ' . __('MatraqaTec'))
...
<div class="space-y-8 animate-in fade-in slide-in-from-bottom duration-700">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-black text-slate-800 dark:text-white">{{ __('Add New FAQ') }}</h1>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-white dark:bg-[#1A1A31] rounded-[2.5rem] p-10 shadow-sm border border-slate-100 dark:border-white/5">
        <form action="{{ route('admin.faqs.store') }}" method="POST" class="space-y-12">
            @csrf
            ...
            <div class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-3 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                        ...
                        <input type="text" name="question_ar" value="{{ old('question_ar') }}" placeholder="{{ __('Enter question') }}" required class="w-full bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 dark:text-white placeholder-slate-300 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                    ...
                    <div class="space-y-3 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                        ...
                        <input type="text" name="question_en" value="{{ old('question_en') }}" placeholder="{{ __('Enter question') }}" required class="w-full bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 dark:text-white placeholder-slate-300 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-4" x-data="{ open: false, selected: '{{ old('target_group', 'all') }}' }">
                ...
                <!-- Status Dropdown -->
                <div class="space-y-4" x-data="{ open: false, selected: '{{ old('status', 'active') }}' }">
                    <label class="block text-sm font-black text-slate-700 dark:text-slate-300 flex items-center gap-2">
