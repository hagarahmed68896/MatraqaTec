...
<div x-data="{ 
    ...
    updateChart(type) {
        this.chartType = type;
        fetch(`{{ route('admin.individual-customers.show', $item->id) }}?chart_type=${type}`, {
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.individual-customers.index') }}" class="w-10 h-10 rounded-xl bg-white dark:bg-white/5 flex items-center justify-center text-slate-400 hover:text-primary dark:hover:text-white transition-all shadow-sm">
...
<h2 class="text-2xl font-black text-slate-800 dark:text-white">
    ...
    <div class="flex items-center p-1 bg-slate-100 dark:bg-white/5 rounded-2xl w-fit overflow-x-auto no-scrollbar max-w-full">
        <button @click="activeTab = 'overview'" :class="activeTab === 'overview' ? 'bg-white dark:bg-[#1A1A31] shadow-sm text-slate-900 dark:text-white' : 'text-slate-400 font-bold'" class="px-6 py-2.5 rounded-xl text-md whitespace-nowrap transition-all font-bold">{{ __('Overview') }}</button>
        <button @click="activeTab = 'orders'" :class="activeTab === 'orders' ? 'bg-white dark:bg-[#1A1A31] shadow-sm text-slate-900 dark:text-white' : 'text-slate-400 font-bold'" class="px-6 py-2.5 rounded-xl text-md whitespace-nowrap transition-all font-bold">{{ __('Orders') }}</button>
        <button @click="activeTab = 'invoices'" :class="activeTab === 'invoices' ? 'bg-white dark:bg-[#1A1A31] shadow-sm text-slate-900 dark:text-white' : 'text-slate-400 font-bold'" class="px-6 py-2.5 rounded-xl text-md whitespace-nowrap transition-all font-bold">{{ __('Invoices') }}</button>
        <button @click="activeTab = 'payments'" :class="activeTab === 'payments' ? 'bg-white dark:bg-[#1A1A31] shadow-sm text-slate-900 dark:text-white' : 'text-slate-400 font-bold'" class="px-6 py-2.5 rounded-xl text-md whitespace-nowrap transition-all font-bold">{{ __('Payments') }}</button>
        <button @click="activeTab = 'ratings'" :class="activeTab === 'ratings' ? 'bg-white dark:bg-[#1A1A31] shadow-sm text-slate-900 dark:text-white' : 'text-slate-400 font-bold'" class="px-6 py-2.5 rounded-xl text-md whitespace-nowrap transition-all font-bold">{{ __('Ratings') }}</button>
    </div>
        ...
        <div x-show="activeTab === 'overview'" x-transition class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="space-y-8">
                <div class="bg-white dark:bg-[#1A1A31] p-8 rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-sm text-center relative overflow-hidden">
                    <div class="absolute top-0 left-0 w-full h-24 dark:bg-primary/5 bg-[#1A1A31]"></div>
...
<h3 class="text-xl font-black text-slate-800 dark:text-white">
    ...
    @endif
                        ...
                        <div class="mt-8 space-y-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                            ...
                            <div class="flex items-center gap-4 group mb-1">
                                ...
                                <div class="flex-1 min-w-0 flex items-center gap-1">
                                    ...
                                    <p class="text-md font-black text-slate-700 dark:text-white/80 truncate">{{ __('Individual Customer') }}</p>
                                </div>
                            </div>
        ...
        <div class="flex-1 min-w-0 flex items-center gap-1">
            <p class="text-[12px] font-bold text-slate-400 uppercase tracking-widest shrink-0">{{ __('Email') }}:</p>
            <p class="text-md font-black text-slate-700 dark:text-white/80 truncate">{{ $item->user->email }}</p>
        </div>
    </div>
        ...
        <div class="flex-1 min-w-0 flex items-center gap-1">
            <p class="text-[12px] font-bold text-slate-400 uppercase tracking-widest shrink-0">{{ __('Phone Number') }}:</p>
            <p class="text-md font-black text-slate-700 dark:text-white/80" dir="ltr">{{ $item->user->phone }}</p>
        </div>
    </div>
             ...
             <!-- Address -->
                            <div class="flex items-center gap-4 group mb-1">
                                ...
                                <div class="flex-1 min-w-0 flex items-center gap-1">
                                    ...
                                    <p class="text-md font-black text-slate-700 dark:text-white/80 truncate">{{ $item->address ?? '-' }}</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-4 group mb-1">
                                ...
                                <div class="flex-1 min-w-0 flex items-center gap-1">
                                    ...
                                    <span class="px-2 py-0.5 rounded-lg text-[12px] font-black uppercase tracking-wider {{ $item->user->status == 'active' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500' }}">
                                        {{ __($item->user->status) }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-4 group mb-1">
                                ...
                                <div class="flex-1 min-w-0 flex items-center gap-1">
                                    ...
                                    <p class="text-md font-black text-slate-700 dark:text-white/80 truncate">{{ $item->created_at->format('Y-m-d') }}</p>
                                </div>
                            </div>
                        </div>

                    <div class="mt-10 flex flex-col gap-3">
    <a href="{{ route('admin.individual-customers.edit', $item->id) }}" class="w-full py-4 bg-primary text-white rounded-2xl font-black text-xs shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all flex items-center justify-center gap-2">
        {{ __('Edit Profile') }}
    </a>

    <form action="{{ route('admin.individual-customers.toggle-block', $item->id) }}" method="POST" class="w-full">
        @csrf
        <button type="submit" 
            ...
            {{ $item->user->status == 'active' 
                ? 'border-rose-500 text-rose-500  shadow-lg shadow-rose-500/10' 
                : 'border-emerald-500 text-emerald-500 shadow-lg shadow-emerald-500/10' 
            }}">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="bg-white dark:bg-[#1A1A31] p-6 rounded-[2rem] border border-slate-100 dark:border-white/5 shadow-sm">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ __('Total Paid') }}</span>
                ...
                <div class="bg-white dark:bg-[#1A1A31] p-8 rounded-[2rem] border border-slate-100 dark:border-white/5 shadow-sm">
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-lg font-black text-slate-800 dark:text-white">{{ __('Service Performance') }}</h3>
                ...
                <div class="bg-white dark:bg-[#1A1A31] p-8 rounded-[2rem] border border-slate-100 dark:border-white/5 shadow-sm">
                    <h3 class="text-lg font-black text-slate-800 dark:text-white mb-6">{{ __('Recent Orders') }}</h3>
                    <div class="space-y-4">
                        @forelse($orders->take(5) as $order)
                        <div class="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-white/5 transition-all">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-xl bg-white dark:bg-white/10 flex items-center justify-center text-primary font-black shadow-sm">
                                    #{{ mb_substr($order->order_number, -2) }}
                                </div>
                                <div>
                                    <h4 class="text-sm font-black text-slate-800 dark:text-white">{{ $order->service->{'name_'.app()->getLocale()} ?? '-' }}</h4>
                        ...
                        @empty
                        <p class="text-center text-slate-400 py-8">{{ __('No recent orders') }}</p>
                        @endforelse
                    </div>
                </div>
            </div>
        ...
        <div x-show="activeTab === 'orders'" x-transition class="space-y-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h3 class="text-xl font-black text-slate-800 dark:text-white">{{ __('Order History') }}</h3>
        ...
        <div x-show="activeTab === 'payments'" x-transition class="space-y-6">
            <div class="flex items-center justify-between">
                <h3 class="text-xl font-black text-slate-800 dark:text-white">{{ __('Payment History') }}</h3>
                <div class="relative w-full md:w-80">
                    <span class="absolute inset-y-0 left-{{ app()->getLocale() == 'ar' ? 'auto right' : '0' }}-0 {{ app()->getLocale() == 'ar' ? 'pr' : 'pl' }}-4 flex items-center text-slate-400">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    </span>
        ...
        <!-- Ratings Tab -->
        <div x-show="activeTab === 'ratings'" x-transition class="space-y-6">
