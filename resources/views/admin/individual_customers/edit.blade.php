...
@section('page_title', __('Edit Account Data'))
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white dark:bg-[#1A1A31] rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-white/5 overflow-hidden">
        <form action="{{ route('admin.individual-customers.update', $item->id) }}" method="POST" class="p-8 md:p-12">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-8">
                <!-- Name Field -->
                <div class="space-y-3">
                    <label class="block text-sm font-black text-slate-700 dark:text-slate-300 pr-2">{{ __('Name') }}</label>
                    <input type="text" 
                           name="name" 
    ...
    <div class="flex items-center w-full bg-slate-50 dark:bg-white/5 border {{ $errors->has('phone') ? 'border-red-500' : 'border-transparent' }} focus-within:border-primary focus-within:bg-white dark:focus-within:bg-[#1A1A31] rounded-2xl transition-all overflow-hidden" dir="ltr">
        <div class="pl-6 pr-4 flex items-center justify-center border-r border-slate-200 dark:border-white/10 self-stretch">
            <span class="text-slate-400 font-bold text-sm whitespace-nowrap">+966</span>
        </div>

        <input type="text" 
               name="phone" 
               value="{{ old('phone', isset($item) ? $item->user->phone : '') }}" 
               placeholder="5XXXXXXXX"
               class="flex-1 px-4 py-4 bg-transparent border-none text-sm font-bold text-slate-700 dark:text-white placeholder-slate-400 transition-all outline-none"
               style="text-align: left;">
    </div>

    @error('phone') 
        <p class="text-red-500 text-xs mt-1 font-bold text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
            {{ $message }}
        </p> 
    @enderror
</div>
