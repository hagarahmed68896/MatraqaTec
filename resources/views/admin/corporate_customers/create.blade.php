...
@section('page_title', __('Add New Corporate Customer'))
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-black text-slate-800 dark:text-white">{{ __('Add New Corporate Customer') }}</h1>
        </div>
    </div>

    <!-- Main Card -->
    <div class="bg-white dark:bg-[#1A1A31] rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-white/5 overflow-hidden">
        <form action="{{ route('admin.corporate-customers.store') }}" method="POST" enctype="multipart/form-data" class="p-8 md:p-12">
            @csrf
            ...
            <div class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-3 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                        ...
                        <input type="text" 
                               ...
                               value="{{ old('commercial_record_number') }}" 
                    ...
                    <!-- CR File Upload -->
                    <div class="space-y-3 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}" x-data="{ fileName: '{{ __('No file selected yet') }}' }">
                ...
                <div class="space-y-4 pt-4">
                    ...
                    <div class="relative" x-data="{ 
                        ...
                        statusLabel: '{{ old('status', 'active') == 'active' ? __('نشط') : __('غير نشط') }}'
                    }">
                        <!-- Dropdown Button -->
                        <button type="button" 
                                @click="open = !open"
