...
@section('title', __('Edit Role') . ': ' . ($item->name_ar ?? $item->name))
...
<div x-data="{
    ...
    isGroupAssigned(group) {
        return this.groupMap[group].some(id => this.permissions.includes(id));
    },

    isGroupVisible(group) {
        return !this.filterActive || this.isGroupAssigned(group);
    },
    ...
    <div class="flex items-center justify-between gap-4 mb-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-black text-slate-800 dark:text-white">{{ __('Edit Role') }}</h1>
                <p class="text-slate-500 dark:text-slate-400 font-bold text-sm">{{ $item->name_ar ?? $item->name }}</p>
            </div>
        </div>
    </div>

    <form action="{{ route('admin.roles.update', $item->id) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-3">
                    ...
                    <textarea name="description_ar" rows="4" placeholder="{{ __('Enter description') }}"
                              class="w-full px-6 py-4 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl focus:outline-none focus:ring-2 focus:ring-primary/20 dark:text-white text-md font-bold transition-all resize-none">{{ old('description_ar', $item->description_ar) }}</textarea>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <textarea name="description_en" rows="4" placeholder="{{ __('Enter description') }}"
                              class="w-full px-6 py-4 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl focus:outline-none focus:ring-2 focus:ring-primary/20 dark:text-white text-md font-bold transition-all resize-none text-left" dir="ltr">{{ old('description_en', $item->description_en) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-[#1A1A31] rounded-[2.5rem] p-8 lg:p-10 border border-slate-100 dark:border-white/5 shadow-sm overflow-hidden">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-6 mb-8">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-xl font-black text-slate-800 dark:text-white">{{ __('Specialties & Permissions') }}</h2>
                    <template x-if="filterActive">
                        <span class="px-3 py-1 bg-primary/10 text-primary text-[10px] font-black rounded-full uppercase">{{ __('Assigned Specialties Only') }}</span>
                    </template>
                </div>
                
                <div class="flex items-center gap-4">
                    <!-- Filter Toggle -->
                    <button type="button" @click="filterActive = !filterActive" 
                            class="px-5 py-2.5 rounded-xl border border-slate-200 dark:border-white/10 text-xs font-bold transition-all"
                            :class="filterActive ? 'bg-white dark:bg-white/5 text-slate-600 dark:text-slate-400' : 'bg-[#1A1A31] text-white shadow-lg'">
                        <span x-text="filterActive ? '{{ __('Show All Specialties') }}' : '{{ __('Focus on Assigned') }}'"></span>
                    </button>

                    <div class="flex items-center gap-4 bg-slate-50 dark:bg-white/5 px-6 py-2.5 rounded-2xl border border-slate-100 dark:border-white/5">
                        <span class="text-sm font-black text-slate-700 dark:text-slate-300">{{ __('Full Access') }}</span>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
                @foreach($groupedModules as $groupName => $modules)
                    ...
                    @endphp
                    <div x-show="isGroupVisible('{{ $groupName }}')"
                         x-transition
                         @click="masterGroups['{{ $groupName }}'] = !masterGroups['{{ $groupName }}']; toggleGroup('{{ $groupName }}')"
                ...
                <div x-show="showAdvanced" x-collapse x-transition:enter="transition ease-out duration-300" class="w-full mt-10">
                    <div class="overflow-x-auto rounded-[2rem] border border-slate-100 dark:border-white/5">
                        <table class="permissions-table w-full">
                            ...
                            <tbody>
                                @foreach($groupedModules as $groupName => $modules)
                                    <tr class="group-header" x-show="isGroupVisible('{{ $groupName }}')">
                                    ...
                                    @foreach($modules as $moduleName => $moduleKeys)
                                        <tr x-show="isGroupVisible('{{ $groupName }}')">
