...
<div class="max-w-4xl mx-auto" dir="rtl">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <h2 class="text-2xl font-black text-[#1A1A31] dark:text-white">{{ __('Update Item') }}</h2>
    </div>

    <form action="{{ route('admin.inventory.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-[#1A1A31] rounded-[2.5rem] p-10 shadow-sm border border-slate-50 dark:border-white/5 space-y-10">
            ...
            <div class="space-y-4">
                ...
                <div id="drop-area" class="relative group">
                    ...
                    {{-- Dropzone State (Hidden if image exists) --}}
                    <div id="dropzone-empty" class="{{ $item->image ? 'hidden' : '' }} h-24 w-full rounded-2xl bg-[#F8F9FE] dark:bg-white/5 border border-dashed border-slate-200 dark:border-white/10 flex items-center justify-center transition-all group-hover:border-[#1A1A31]/20 dark:group-hover:border-white/20">
                    ...
                    {{-- Preview State --}}
                    <div id="dropzone-preview" class="{{ $item->image ? '' : 'hidden' }} h-24 w-full rounded-2xl bg-[#F8F9FE] dark:bg-white/5 border border-slate-200 dark:border-white/10 px-6 flex items-center justify-between transition-all">
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 rounded-xl bg-white dark:bg-white/10 p-2 flex items-center justify-center shadow-sm overflow-hidden">
                                <img id="preview-img" src="{{ $item->image ? asset($item->image) : '' }}" class="w-full h-full object-contain">
                            </div>
                            <div class="text-right">
                                <p id="file-name" class="text-xs font-black text-[#1A1A31] dark:text-white truncate max-w-[200px]">
                                    {{ $item->image ? basename($item->image) : '' }}
                                </p>
                                <p id="file-size" class="text-[10px] font-bold text-slate-400 uppercase">
                                    {{ $item->image && file_exists(public_path($item->image)) ? number_format(filesize(public_path($item->image)) / 1024 / 1024, 2) . ' MB' : '' }}
                                </p>
                            </div>
                        </div>
            ...
            <div class="grid mb-4 grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-4">
                    ...
                    <input type="text" name="name_ar" value="{{ $item->name_ar }}" placeholder="{{ __('Enter item name') }}" required class="w-full px-6 py-4 rounded-2xl bg-[#F8F9FE] dark:bg-white/5 border border-slate-100 dark:border-white/10 text-sm font-bold text-[#1A1A31] dark:text-white focus:ring-4 focus:ring-[#1A1A31]/5 transition-all outline-none">
                </div>
                <div class="space-y-4">
                    <label class="block text-sm font-black text-[#1A1A31] dark:text-slate-300 text-left">{{ __('Item Name (English)') }}</label>
                    <input type="text" name="name_en" value="{{ $item->name_en }}" placeholder="{{ __('Enter item name') }}" required class="w-full px-6 py-4 rounded-2xl bg-[#F8F9FE] dark:bg-white/5 border border-slate-100 dark:border-white/10 text-sm font-bold text-[#1A1A31] dark:text-white focus:ring-4 focus:ring-[#1A1A31]/5 transition-all outline-none text-left">
                </div>
            </div>
            ...
            <div class="grid mb-4 grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-4">
                    ...
                    <div class="relative">
                        <input type="number" step="0.01" name="price" value="{{ $item->price }}" placeholder="{{ __('0.00') }}" required class="w-full px-6 py-4 rounded-2xl bg-[#F8F9FE] dark:bg-white/5 border border-slate-100 dark:border-white/10 text-sm font-bold text-[#1A1A31] dark:text-white focus:ring-4 focus:ring-[#1A1A31]/5 transition-all outline-none pl-14 pr-6">
                ...
                <div class="space-y-4" x-data="{ status: '{{ $item->status }}' }">
...
<script>
    ...
    function clearImage() {
        ...
        @endif
        document.getElementById('preview-img').src = '{{ $item->image ? asset($item->image) : "" }}';
        document.getElementById('file-name').textContent = '{{ $item->image ? basename($item->image) : "" }}';
    }
</script>
@endsection
