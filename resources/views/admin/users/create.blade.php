...
@section('title', __('Create User') . ' - ' . __('MatraqaTec'))
    ...
    <div class="bg-white dark:bg-[#1A1A31] rounded-[2rem] border border-slate-100 dark:border-white/5 shadow-sm p-8">
        <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="phone" name="phone" value="{{ old('phone') }}" required maxlength="9" inputmode="numeric"
                ...
                <p class="text-xs text-red-500 font-bold">{{ $message }}</p>
                @enderror
                <p class="text-xs text-slate-500 dark:text-white/50">{{ __('9 digits without country code') }}</p>
            </div>
            ...
            <div class="space-y-2">
                ...
                <p class="text-xs text-red-500 font-bold">{{ $message }}</p>
                @enderror
                <p class="text-xs text-slate-500 dark:text-white/50">{{ __('Minimum 6 characters') }}</p>
            </div>
            ...
            <div class="space-y-2">
                ...
                <select id="type" name="type" required
                    ...
                    <option value="technician" {{ old('type') == 'technician' ? 'selected' : '' }}>{{ __('Technician') }}</option>
                    <option value="supervisor" {{ old('type') == 'supervisor' ? 'selected' : '' }}>{{ __('Supervisor') }}</option>
