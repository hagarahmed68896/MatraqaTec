...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-2xl font-black text-slate-800 dark:text-white">{{ __('Create New Service') }}</h2>
            <p class="text-slate-500 dark:text-slate-400 text-sm font-bold mt-1">{{ __('Add a new service to the platform') }}</p>
        </div>
    ...
    <!-- Form -->
    <form action="{{ route('admin.services.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" x-data="{ 
        subServices: [],
        addSubService() {
        ...
        <div class="bg-white dark:bg-[#1A1A31] rounded-2xl p-6 border border-slate-100 dark:border-white/5 shadow-sm space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name_ar" value="{{ old('name_ar') }}" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-white/5 border-none focus:ring-2 focus:ring-primary text-slate-800 dark:text-white text-sm font-bold" required>
                ...
                <div>
                    ...
                    <input type="text" name="name_en" value="{{ old('name_en') }}" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-white/5 border-none focus:ring-2 focus:ring-primary text-slate-800 dark:text-white text-sm font-bold" required>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description_ar" rows="3" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-white/5 border-none focus:ring-2 focus:ring-primary text-slate-800 dark:text-white text-sm font-bold">{{ old('description_ar') }}</textarea>
                </div>

                <!-- Description EN -->
                <div class="md:col-span-2">
                    <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-2 uppercase">{{ __('Description (English)') }}</label>
                    <textarea name="description_en" rows="3" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-white/5 border-none focus:ring-2 focus:ring-primary text-slate-800 dark:text-white text-sm font-bold">{{ old('description_en') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" step="0.01" name="price" value="{{ old('price') }}" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-white/5 border-none focus:ring-2 focus:ring-primary text-slate-800 dark:text-white text-sm font-bold">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative w-full h-48 rounded-xl bg-slate-50 dark:bg-white/5 border-2 border-dashed border-slate-200 dark:border-white/10 flex flex-col items-center justify-center text-center cursor-pointer hover:border-primary transition-colors group overflow-hidden">
                        ...
                        <img id="mainImagePreview" class="absolute inset-0 w-full h-full object-cover opacity-0 transition-opacity z-0">
                        <div class="z-0 pointer-events-none group-hover:scale-110 transition-transform duration-300">
                ...
                <div>
                    ...
                    <div class="relative w-full h-48 rounded-xl bg-slate-50 dark:bg-white/5 border-2 border-dashed border-slate-200 dark:border-white/10 flex flex-col items-center justify-center text-center cursor-pointer hover:border-primary transition-colors group overflow-hidden">
                        ...
                        <img id="iconPreview" class="absolute inset-0 w-full h-full object-contain p-4 opacity-0 transition-opacity z-0">
                        <div class="z-0 pointer-events-none group-hover:scale-110 transition-transform duration-300">
        ...
        <div class="bg-white dark:bg-[#1A1A31] rounded-2xl p-6 border border-slate-100 dark:border-white/5 shadow-sm space-y-6">
            ...
            <div class="space-y-4">
                <template x-for="(sub, index) in subServices" :key="sub.id">
