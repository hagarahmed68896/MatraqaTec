...
@section('title', __('Blocked Supervisors'))
...
<div class="space-y-6 pb-20" x-data="{ 
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 overflow-visible">
        <div>
            <h1 class="text-3xl font-black text-slate-800 dark:text-white mb-2 tracking-tight">{{ __('Blocked Supervisors') }}</h1>
            <p class="text-slate-500 dark:text-slate-400 font-bold text-sm">{{ __('Manage blocked system supervisors.') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#1A1A31] rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-none overflow-hidden transition-all duration-500" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
        <div class="p-8 flex flex-col md:flex-row items-center justify-between gap-6 bg-slate-50/50 dark:bg-white/[0.02] border-b border-slate-100 dark:border-white/5">
             ...
             <div class="flex items-center gap-3 w-full md:w-auto" x-show="selectedIds.length === 0">
                <form action="{{ url()->current() }}" method="GET" class="relative group" x-data="{ search: '{{ request('search') }}' }">
                    <input type="text"
                        name="search"
                        x-model="search"
                        placeholder="{{ __('Search supervisor by name or email...') }}"
            ...
            <div class="flex items-center gap-3 shrink-0" x-show="selectedIds.length === 0">
                <a href="{{ route('admin.blocked.download', ['target' => 'supervisors']) }}" 
        ...
        <div class="overflow-x-auto overflow-y-visible">
             <table class="w-full text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                ...
                <tr class="text-slate-400 text-[11px] font-black uppercase tracking-[0.2em] border-b border-slate-100 dark:border-white/5 bg-slate-50/30 dark:bg-white/[0.01]">
                    ...
                    <th class="py-5 px-6">{{ __('Supervisor Name') }}</th>
                    <th class="py-5 px-6">{{ __('Mobile Number') }}</th>
                    <th class="py-5 px-6">{{ __('Email') }}</th>
                    <th class="py-5 px-6">{{ __('Role') }}</th>
                ...
                <tbody class="text-sm font-bold text-slate-600 dark:text-slate-300">
                    ...
                    <tr class="border-b border-slate-50 dark:border-white/[0.02] hover:bg-slate-50/80 dark:hover:bg-white/[0.03] transition-all duration-300 group">
                        <td class="py-6 px-8 text-center">
                            <input type="checkbox" x-model="selectedIds" value="{{ $item->id }}" class="row-checkbox w-5 h-5 border-2 border-slate-200 dark:border-white/10 rounded-lg text-primary focus:ring-primary focus:ring-offset-0 transition-all cursor-pointer">
                        </td>
                        <td class="py-6 px-6 whitespace-nowrap">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex flex-col">
                                    <span class="text-slate-900 dark:text-white font-black text-sm">{{ $item->name }}</span>
                                    <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">ID: #{{ $item->id }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="py-6 px-6 whitespace-nowrap">
                            <span class="font-mono opacity-80 text-xs" dir="ltr">{{ $item->phone ?? '-' }}</span>
                        </td>
                        <td class="py-6 px-6">
                            <span class="opacity-80 text-xs truncate max-w-[150px] inline-block">{{ $item->email ?? '-' }}</span>
                        </td>
                        <td class="py-6 px-6">
                            <div class="flex flex-wrap gap-1">
                                @forelse($item->roles as $role)
                                    <span class="px-2.5 py-1 rounded-lg bg-primary/5 text-primary text-[10px] font-black uppercase tracking-tight">{{ $role->name }}</span>
                                @empty
                                    <span class="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-white/5 text-slate-400 text-[10px] font-black uppercase tracking-tight">{{ __('No Role') }}</span>
                                @endforelse
                            </div>
                        </td>
                        <td class="py-6 px-6 whitespace-nowrap">
                    ...
                    <tr>
                        <td colspan="7" class="py-24 text-center">
                             <div class="flex flex-col items-center max-w-sm mx-auto">
                                <div class="w-24 h-24 bg-slate-50 dark:bg-white/5 rounded-[2.5rem] flex items-center justify-center mb-6 text-slate-200 dark:text-slate-700 transform rotate-[10deg]">
                                    <svg class="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/>
                                    </svg>
                                </div>
                                <h3 class="font-black text-xl text-slate-800 dark:text-white mb-2">{{ __('No blocked supervisors found') }}</h3>
                                <p class="text-slate-400 font-bold text-sm">{{ __('When a supervisor is blocked, they will appear in this list.') }}</p>
                             </div>
                        </td>
                    </tr>
                    @endforelse
