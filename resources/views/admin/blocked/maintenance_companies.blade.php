...
@section('title', __('Blocked Companies'))
...
<div class="space-y-6 pb-20" x-data="{ 
    ...
    async unblockSelected() {
        if (!confirm('{{ __('Are you sure you want to unblock the selected companies?') }}')) return;
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 overflow-visible">
        <div>
            <h1 class="text-3xl font-black text-slate-800 dark:text-white mb-2 tracking-tight">{{ __('Blocked Companies') }}</h1>
            <p class="text-slate-500 dark:text-slate-400 font-bold text-sm">{{ __('Manage blocked maintenance companies.') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#1A1A31] rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-none overflow-hidden transition-all duration-500" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
        <div class="p-8 flex flex-col md:flex-row items-center justify-between gap-6 bg-slate-50/50 dark:bg-white/[0.02] border-b border-slate-100 dark:border-white/5">
             ...
             <div class="flex items-center gap-3 w-full md:w-auto" x-show="selectedIds.length === 0">
                <form action="{{ url()->current() }}" method="GET" class="relative group" x-data="{ search: '{{ request('search') }}' }">
                    <input type="text"
                        name="search"
                        x-model="search"
                        placeholder="{{ __('Search company by name or phone...') }}"
            ...
            <div class="flex items-center gap-3 shrink-0" x-show="selectedIds.length === 0">
                <a href="{{ route('admin.blocked.download', ['target' => 'companies']) }}" 
             ...
             <table class="w-full text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                ...
                <tbody class="text-sm font-bold text-slate-600 dark:text-slate-300">
                    ...
                    <tr class="border-b border-slate-50 dark:border-white/[0.02] hover:bg-slate-50/80 dark:hover:bg-white/[0.03] transition-all duration-300 group">
                        ...
                        <td class="py-6 px-6">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary font-black text-md shadow-sm group-hover:scale-110 transition-transform duration-300">
                                    {{ mb_substr($item->name ?? $item->company_name_ar, 0, 1) }}
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-slate-900 dark:text-white font-black text-sm whitespace-nowrap">{{ $item->name ?? $item->company_name_ar }}</span>
                        ...
                        <td class="py-6 px-6">
                            <span class="opacity-80 text-xs truncate max-w-[150px] inline-block">{{ $item->user->email ?? '-' }}</span>
                        </td>
                        <td class="py-6 px-6">
                        ...
                        <td class="py-6 px-8 text-center">
                            <form action="{{ route('admin.blocked.bulk-unblock') }}" method="POST" onsubmit="return confirm('{{ __('Are you sure you want to unblock this company?') }}')">
                                @csrf
                                <input type="hidden" name="ids[]" value="{{ $item->user_id }}">
                                <input type="hidden" name="target_type" value="company">
                    ...
                    <tr>
                        <td colspan="8" class="py-24 text-center">
                             <div class="flex flex-col items-center max-w-sm mx-auto">
                                <div class="w-24 h-24 bg-slate-50 dark:bg-white/5 rounded-[2.5rem] flex items-center justify-center mb-6 text-slate-200 dark:text-slate-700 transform -rotate-6">
                                    <svg class="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                                    </svg>
                                </div>
                                <h3 class="font-black text-xl text-slate-800 dark:text-white mb-2">{{ __('No blocked companies found') }}</h3>
                                <p class="text-slate-400 font-bold text-sm">{{ __('When a company is blocked, they will appear in this list.') }}</p>
                             </div>
                        </td>
                    </tr>
                    @endforelse
