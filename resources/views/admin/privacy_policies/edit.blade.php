...
@section('title', __('Edit Policy Data') . ' - ' . __('MatraqaTec'))
...
<div class="space-y-8 animate-in fade-in slide-in-from-bottom duration-700">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-black text-slate-800 dark:text-white">{{ __('Edit Policy Data') }}</h1>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-white dark:bg-[#1A1A31] rounded-[2.5rem] p-10 shadow-sm border border-slate-100 dark:border-white/5">
        <form action="{{ route('admin.privacy-policies.update', $item->id) }}" method="POST" class="space-y-10">
            @csrf
            @method('PUT')
