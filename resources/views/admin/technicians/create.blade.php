...
@section('title', __('New Technician'))
@section('page_title', __('New Technician'))
...
<div class="max-w-4xl mx-auto pb-20" x-data="{ 
    ...
    <form action="{{ route('admin.technicians.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
            ...
            <div class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                    <div class="space-y-3">
                        ...
                        <input type="number" name="years_experience" value="{{ old('years_experience', 0) }}" 
                               class="w-full px-6 py-4 bg-slate-50 dark:bg-white/5 border border-transparent focus:border-primary focus:bg-white dark:focus:bg-[#1A1A31] rounded-2xl text-sm font-bold transition-all outline-none">
                    </div>
                    <div class="space-y-3">
                        <label class="block text-sm font-black text-slate-700 dark:text-slate-300 pr-2">{{ __('Bio (English)') }}</label>
                        <textarea name="bio_en" rows="3" dir="ltr" class="w-full px-6 py-4 bg-slate-50 dark:bg-white/5 border border-transparent focus:border-primary focus:bg-white dark:focus:bg-[#1A1A31] rounded-2xl text-sm font-bold transition-all outline-none resize-none">{{ old('bio_en') }}</textarea>
                    </div>
                    <div class="md:col-span-2 space-y-3">
                        <label class="block text-sm font-black text-slate-700 dark:text-slate-300 pr-2">{{ __('Bio (Arabic)') }}</label>
                        <textarea name="bio_ar" rows="3" class="w-full px-6 py-4 bg-slate-50 dark:bg-white/5 border border-transparent focus:border-primary focus:bg-white dark:focus:bg-[#1A1A31] rounded-2xl text-sm font-bold transition-all outline-none resize-none">{{ old('bio_ar') }}</textarea>
                    </div>
                </div>
