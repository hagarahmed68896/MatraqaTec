...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-white/50">{{ __('Manage partner maintenance companies and their staff') }}</p>
        </div>
        <a href="{{ route('admin.maintenance-companies.create') }}" class="px-6 py-3 bg-primary text-white rounded-xl font-bold hover:bg-primary-light transition-all flex items-center gap-2">
    ...
    <div class="bg-white dark:bg-[#1A1A31] rounded-[2rem] border border-slate-100 dark:border-white/5 p-6">
        <form action="{{ route('admin.maintenance-companies.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Search name, email...') }}" class="px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 focus:outline-none focus:ring-2 focus:ring-primary/20 dark:text-white">
            ...
            <table class="w-full text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                <thead>
                    <tr class="text-slate-400 text-[10px] font-black uppercase tracking-widest border-b border-slate-100 dark:border-white/5">
                        ...
                        <th class="pb-4 px-6">{{ __('Company Name') }}</th>
                        <th class="pb-4 px-6">{{ __('Email') }}</th>
                        <th class="pb-4 px-6">{{ __('Phone') }}</th>
                        <th class="pb-4 px-6">{{ __('Services') }}</th>
                ...
                <tbody class="text-xs font-bold text-slate-600 dark:text-white/70">
                    ...
                    <tr onclick="window.location='{{ route('admin.maintenance-companies.show', $item->id) }}'" class="border-b border-slate-50 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 transition-all cursor-pointer">
                        ...
                        <td class="py-4 px-6">
                            <span class="px-2 py-1 rounded-lg bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400">
                                {{ $item->services->count() }} {{ __('Services') }}
                            </span>
                        </td>
                        <td class="py-4 px-6">
                        ...
                        <td class="py-4 px-6">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.maintenance-companies.show', $item->id) }}" class="p-2 rounded-lg hover:bg-blue-50 text-blue-600">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </a>
                                <a href="{{ route('admin.maintenance-companies.edit', $item->id) }}" class="p-2 rounded-lg hover:bg-yellow-50 text-yellow-600">
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="py-12 text-center text-slate-400">{{ __('No companies found') }}</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
