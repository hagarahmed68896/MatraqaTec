...
@section('page_title', __('Add New Supervisor'))

@section('content')
<div class="max-w-4xl mx-auto" x-data="{ 
    openStatus: false,
    status: '{{ old('status', 'active') }}',
    statusLabel: '{{ old('status', 'active') == 'active' ? __('Active') : __('Blocked') }}'
}">
    ...
    <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-black text-slate-800 dark:text-white">{{ __('Add New Supervisor') }}</h1>
        </div>
    </div>

    <!-- Main Card -->
    <div class="bg-white dark:bg-[#1A1A31] rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-white/5 overflow-hidden">
        <form action="{{ route('admin.supervisors.store') }}" method="POST" class="p-8 md:p-12">
            @csrf
            ...
            <div class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="first_name" value="{{ old('first_name') }}" required placeholder="{{ __('Enter first name') }}"
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="last_name" value="{{ old('last_name') }}" required placeholder="{{ __('Enter last name') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" required placeholder="{{ __('Enter email address') }}"
