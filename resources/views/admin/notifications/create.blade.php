...
@section('title', __('Add Notification'))
@section('page_title', __('Add Notification'))
...
<div class="max-w-4xl mx-auto pb-20" x-data="{ 
    ...
    <div class="flex items-center justify-between mb-8">
        <a href="{{ route('admin.broadcast-notifications.index') }}" class="flex items-center gap-2 text-slate-500 hover:text-primary transition-colors font-bold group">
            ...
            <span class="text-xl font-black text-slate-800 dark:text-white">{{ __('Add Notification') }}</span>
        </a>
    </div>

    <form action="{{ route('admin.broadcast-notifications.store') }}" method="POST" class="space-y-8">
        @csrf
            ...
            <div class="space-y-8">
                    ...
                    <div class="space-y-4">
                        ...
                        <select name="type" required 
                            ...
                            @foreach(['alert', 'reminder', 'notification'] as $t)
                                <option value="{{ $t }}" {{ old('type') == $t ? 'selected' : '' }}>{{ __($t) }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-3">
                        ...
                        <input type="text" name="title_ar" value="{{ old('title_ar') }}" required placeholder="{{ __('Enter Title') }}"
                    ...
                    <div class="space-y-3 text-left">
                        ...
                        <input type="text" name="title_en" value="{{ old('title_en') }}" required dir="ltr" placeholder="{{ __('Enter Title') }}"
                               class="w-full px-6 py-4 bg-slate-50 dark:bg-white/5 border border-transparent focus:border-primary focus:bg-white dark:focus:bg-[#1A1A31] rounded-2xl text-sm font-bold transition-all outline-none">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-3">
                        ...
                        <textarea name="body_ar" rows="4" required placeholder="{{ __('Enter Body') }}"
                                  class="w-full px-6 py-4 bg-slate-50 dark:bg-white/5 border border-transparent focus:border-primary focus:bg-white dark:focus:bg-[#1A1A31] rounded-2xl text-sm font-bold transition-all outline-none resize-none">{{ old('body_ar') }}</textarea>
                    </div>
                    <div class="space-y-3 text-left">
                        ...
                        <textarea name="body_en" rows="4" required dir="ltr" placeholder="{{ __('Enter Body') }}"
                                  class="w-full px-6 py-4 bg-slate-50 dark:bg-white/5 border border-transparent focus:border-primary focus:bg-white dark:focus:bg-[#1A1A31] rounded-2xl text-sm font-bold transition-all outline-none resize-none">{{ old('body_en') }}</textarea>
                    </div>
                </div>
                ...
                <div class="pt-4">
                    ...
                    <select name="status" required 
                        ...
                        @foreach(['sent', 'scheduled', 'unsent'] as $s)
                            <option value="{{ $s }}" {{ old('status') == $s ? 'selected' : '' }}>{{ __($s) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
